...
- 実際の形式だが文章は異なる
  ...
  - 文章: 20yo male with nauseous and abdominal pain since this morning, ate raw oysters yesterday...
...
- labeledが2000に対し、Unlabeledが60000あった
...
- Auxilliary Target
  ...
  - Pseudo Labelingを行いデータを増やした上でのfeatures（コンテキスト文章）の種類の分類

- 特殊なTokenの付与
  - コンテキストIDを入力文章の先頭に加える
  - 改行や医療の略語を特別なトークンで扱う
...
- マーク用Tokenの追加
  - featureテキスト（コンテキストテキスト）を文章の前にマーク用Tokenとして追加した
    - ケースごとに新しいトークンを10種類
      - 例：`[QA CASE=0]`

    - 異なるコンテキストID同士の一部は非常に近いと思った
    - テキストにケース番号の情報を入れることで異なるコンテキストを区別することができるようになると思った
...
- 元データセット + 元の訓練データでラベル付けがされていない6万個ほどのデータ（Hard Pseudo Labeling）
...
2. 訓練データでラベル付けがされていない6万個ほどのデータに対し1. のモデルをEnsembleし、Pseudo Labeling（Hard）
